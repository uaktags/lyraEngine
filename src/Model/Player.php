...
class Player extends Model
{
	...
	public function authenticate($player, $password)
	{
		...
		$pluginData = \App::registerHook('playerLogin', $playerMatch);
	...
	public function create($data)
	{
		...
		$configPasswordStrength = 1;//$this->container['config']['security']['passwordStrength'];
        ...
        $data['active'] = \App::registerHook('playerActivation', $data)[0]; //this is ghetto. Figure out why it's doing this!
	...
	{
		$query = $this->query('SELECT * FROM <prefix>players WHERE lastActive > DATE_SUB(NOW(), INTERVAL 15 MINUTE)');
	...
	{
		$hash = password_verify($input, $existent);
		return $hash;
	}
